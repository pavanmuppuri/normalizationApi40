...
(function (t, e) {
	...
	function s(t, o, n, s) {
		var a = ["opacity", o, ~~(t * 100), n, s].join("-"),
			...
			d = u && "-" + u + "-" || "";
		if (!e[a]) {
			r.insertRule("@" + d + "keyframes " + a + "{" + "0%{opacity:" + l + "}" + f + "%{opacity:" + t + "}" + (f + .01) + "%{opacity:1}" + (f +
				o) % 100 + "%{opacity:" + t + "}" + "100%{opacity:" + l + "}" + "}", r.cssRules.length);
			e[a] = 1
		}
		return a
	}
...
var CoreLoader = (function () {
	...
	function prepareDocument() {
		...
		if (styleTag.styleSheet) {
			styleTag.styleSheet.cssText = _customCSS;
		} else {
			styleTag.appendChild(document.createTextNode(_customCSS));
		}
